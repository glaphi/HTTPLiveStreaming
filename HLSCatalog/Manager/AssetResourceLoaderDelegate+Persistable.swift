...
extension AssetResourceLoaderDelegate {
    ...
    func prepareAndSendPersistableContentKeyRequest(resourceLoadingRequest: AVAssetResourceLoadingRequest) {
        ...
        do {
            ...
            let ckcData = try requestContentKeyFromKeySecurityModule(spcData: spcData, assetID: assetIDString)
            ...
            try writePersistableContentKey(contentKey: persistentKey, withContentKeyIdentifier: assetIDString)
            
            // Provide the content key response to make protected content available for processing.
            resourceLoadingRequest.dataRequest?.respond(with: persistentKey)
            resourceLoadingRequest.finishLoading()
            
            let assetName = contentKeyToStreamNameMap.removeValue(forKey: assetIDString)!
